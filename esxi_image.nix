...
in {
  ...
  options = {
    esxi = {
      ...
      vmx = mkOption {
        ...
        example = "vmx-9";
        description = '' 
          The VMware hardware version, see https://kb.vmware.com/s/articles
        '';
      };
      vmFileName = mkOption {
        type = types.str;
  ...
  config = {
    ...
    system.build.esxi = import "${modulesPath}/../lib/make-disk-image.nix" {
      ...
      postVM =
        ''
          ...
          VBoxManage storageattach "$vmName" --storagectl SCSI --port 0 --device 0 --type hdd \
            --medium disk.vmdk
          ${optionalString (cfg.extraDisk != null) ''
            VBoxManage storageattach "$vmName"j --storagectl SCSI --port 1 --device 0 --type hdd \
